...
float sdf_scene(float3 pos)
{
	...
	float sphere_dist = sdf_sphere(pos, sphere_center, sphere_radius);
	float d = sphere_dist;
...
void main()
{
	...
	float pixel_radius_ws = 0.5 * length(pixel_size_ws);
	...
	while (d < MAX_DIST && inter_steps < 500.0)
	{
		...
		float cone_radius = pixel_radius_ws * d;
		if (dist < cone_radius)
		{
			break;
		}
	...
	if (d < MAX_DIST)
	{
		...
		float aperture = 30.0 * TO_RADIANS;
		float3 cone_dir = N;
		...
		while (true)
		{
			...
			float cone_radius = aperture * t;
			float cone_occlusion = max((cone_radius - dist) / cone_radius, 0.0);
			visibility = visibility * (1.0 - cone_occlusion);

			if (dist < pixel_radius_ws || t >= 2.0 || visibility <= 0.0)
			{
				break;
			}
		...
		R = fract(p);
		R = float3(dot(N, normalize(float3(1.0))));
		R = float3(t / 100.0);
		R = float3(visibility / (visibility + 1.0));
		R = float3(visibility * (N * 0.5 + 0.5));
	}
